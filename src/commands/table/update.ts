...
export default defineLeaf({
  meta: {
    name: "update",
    description: "Update a record. Pass fields via --data '{...}', -f file, or --set field=value",
  },
  args: {
    ...
    file: { type: "string", alias: "f", description: "Path to JSON file (or '-' for stdin)" },
  ...
  async run(ctx, args) {
    ...
    const record = await ctx
      .client()
      .updateRecord(args.table as string, args["sys-id"] as string, data);
