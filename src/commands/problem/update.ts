...
export default defineLeaf({
  meta: { name: "update", description: "Update a problem" },
  args: {
  ...
  async run(ctx, args) {
    ...
    const resolved = await resolveRecordIdentifier(rc, args.id as string, "problem");
    ...
    if (args["known-error"]) data["known_error"] = args["known-error"] === "true";
